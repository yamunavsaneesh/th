...
class Signin extends CI_Controller {
	...
	public function index()
	{
		...
		$this->form_validation->set_rules('huntkey', 'Cleint ID', 'required');
		...
		$login['content']=$this->load->view('admin/login/signin',$login,true);
		$this->load->view('admin/login',$login);
		} else {
			$this->load->model('login_model');
			$user=$this->db->escape_str($this->input->post('login'));
			$pass=$this->db->escape_str($this->input->post('pass'));
			$huntkey=$this->db->escape_str($this->input->post('huntkey'));
			$pass=md5($pass);
			$cond=array('username'=>$user,'password'=>$pass,'huntkey'=>$huntkey);
			...
			if($admin_row){
				...
				$newdata = array(
					   ...
					   'sesshuntkey'  => $admin_row->huntkey,
					   'sesshuntid'  => $admin_row->hunt_id,
	...
	{
		$pass= $this->input->post('pass');$huntkey= $this->input->post('huntkey');
		if (!$this->admin_model->login_check($user,$pass,$huntkey))
		{
