...
class Qas_model extends CI_Model {
	...
	function load($id)
	{
		$this->db->select("$this->table_name.*,$this->sort_table.duration,$this->sort_table.penality");
		if($this->where !='') $this->db->where($this->where);
	...
	{
		if($this->where !='') $this->db->where($this->where);
		$this->db->select("$this->table_name.*,$this->table_name.id as qid,$this->sort_table.sort_order,$this->sort_table.duration,$this->sort_table.penality");
		if(is_array($cond) && count($cond)>0){ 
			$this->db->where($cond);
		}
		$this->db->from($this->table_name);
