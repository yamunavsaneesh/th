...
class Paypal_Lib {
	...
	function Paypal_Lib()
	{
		...
		if($this->CI->config->item('paypal_lib_test_mode')==TRUE){
			$this->paypal_url = 'https://www.sandbox.paypal.com/cgi-bin/webscr';
			$paypalemail=$this->CI->config->item('paypal_lib_test_seller_email');
		} else {
			$this->paypal_url = 'https://www.paypal.com/cgi-bin/webscr';
			$paypalemail=$this->CI->config->item('paypal_lib_seller_email');
		}
		$this->last_error = '';
		$this->ipn_response = '';
		...
		$this->add_field('cmd',$this->CI->config->item('paypal_lib_cart_type'));
		$this->add_field('upload',$this->CI->config->item('paypal_lib_cart_uploads'));
		$this->add_field('business',$paypalemail); // Business Email
	...
	function image($file)
	{
		$this->submit_btn = '<input type="image" name="add" src="' . site_url($this->button_path .'/'.'paypal.jpg') . '" border="0" />';
	}
	...
	function paypal_auto_form() 
	{
		...
		echo '<p style="text-align:center;font-size:20px;font-weight:bold;">Please wait, your order is being processed and you will be redirected to the paypal website.</p>' . "\n";
		echo $this->paypal_form('paypal_auto_form');
		echo "<script>
				$(document).ready(function() {  
				  $('#paypalform').delay(5000).submit();
			   });
			  </script>";
	}
	...
	{
		$str = '';
		$str .= '<form id="paypalform" method="post" action="'.$this->paypal_url.'" name="'.$form_name.'">' . "\n";
